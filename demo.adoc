...
. Create a `notes/src/static.json` file with the configuration for secure headers and redirect all HTTP requests to HTTPS.
+
[source,json]
----
{
...
. Modify `angular.json` to include `static.json` in its assets.
+s
[source,json]
----
"assets": [
