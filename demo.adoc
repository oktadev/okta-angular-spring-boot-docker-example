...
. Configure Angular for OIDC authentication by modifying its `auth-routing.module.ts` to use the generated issuer, client ID, and update the callback URL.
...
. Run the following command in the `notes-api` directory.
+
[source,shell]
----
...
. Add a `HomeController` [`kotlin-hello`]
+
[source,kotlin]
----
...
If you access it using `http` (instead of `https`), it won't work. You'll also score an **F** when you test it using https://securityheaders.com[securityheaders.com].
...
. If you test your new Firebase site on https://securityheaders.com/[securityheaders.com], you'll score a **D**. Luckily, you can https://firebase.google.com/docs/hosting/full-config#headers[configure headers] in your `firebase.json` file.
+
[source,json]
----
"headers": [ {
